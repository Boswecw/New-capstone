  ...
  const fetchSettings = async () => {
    try {
      ...
      if (error.response?.status === 404) {
        console.log('⚙️ Settings endpoint not found, using defaults with current system info...');
        await loadSettingsFromSystem();
      } else {
        showAlert(error.response?.data?.message || 'Failed to load settings', 'warning');
      }
    } finally {
      setLoading(false);
    }
  ...
  const handleSave = async (e) => {
    ...
    try {
      ...
      if (error.response?.status === 404) {
        ...
        showAlert('Settings saved locally (backend endpoint not available)', 'warning');
        setUnsavedChanges(false);
      } else {
        showAlert(error.response?.data?.message || 'Failed to save settings', 'danger');
      }
    } finally {
      setSaving(false);
    }
    ...
    try {
      ...
      const backupData = {
        ...
        summary: {
          ...
          totalContacts: dashboardResponse.data.data.stats.contacts?.totalContacts || 0
        }
      };
      ...
      link.href = url;
      link.download = `furbabies-backup-${new Date().toISOString().split('T')[0]}.json`;
  ...
  return (
    <>
      <div className="d-flex justify-content-between align-items-center mb-4">
        <h1><i className="fas fa-cogs me-2"></i>System Settings</h1>
        <div className="d-flex gap-2">
          ...
          <Button variant="outline-secondary" onClick={() => setShowBackupModal(true)}>
