...
import { getGoogleStorageUrl, handleImageError, generateSrcSet, getDefaultPetImage } from '../utils/imageUtils';
...
const PetCard = ({ pet, onVote, showEditButton = false, showDeleteButton = false, onPetUpdated }) => {
  ...
  // Get the optimized image URL from Google Cloud Storage
  const imageUrl = getGoogleStorageUrl(pet.image, 'medium');
  const imageSrcSet = generateSrcSet(pet.image);
    ...
    <Card className="h-100 pet-card shadow-sm fade-in">
      ...
      <Card.Body className="d-flex flex-column">
        ...
        <div className="d-flex justify-content-between align-items-center mb-3">
          <span className="price fw-bold text-success" style={{ fontSize: '1.2rem' }}>
        ...
        <div className="mt-auto">
          <div className="d-flex gap-2 mb-2">
            ...
            {/* Edit/Delete buttons for pet owners/admins */}
          ...
          {/* Contact/Adopt Button for non-owners */}
          {user && pet.available && user.id !== pet.createdBy?._id && (
            <Link 
              to={`/contact?pet=${pet._id}`}
        ...
        {pet.viewCount > 0 && (
          <div className="text-muted text-center mt-2" style={{ fontSize: '0.75rem' }}>
            <i className="fas fa-eye me-1"></i>
            {pet.viewCount} view{pet.viewCount !== 1 ? 's' : ''}
          </div>
        )}
      </Card.Body>
    </Card>
