  ...
  try {
    ...
    for (let i = 0; i < searchMethods.length; i += 1) {
      try {
        ...
        if (pet) {
          ...
          /* eslint-disable no-console */
          console.log(
            `✅ SUCCESS with "${foundWithMethod}": Found ${pet.name || pet._id}`
          );
          /* eslint-enable no-console */
          break;
        }
    ...
    if (pet && typeof pet.save !== "function") {
      try {
        ...
        /* eslint-disable no-console */
        console.log(
          "⚠️ Could not convert to Mongoose document:",
    ...
    /* eslint-disable no-console */
    console.log(
      `✅ ENHANCED BYPASS SUCCESS: Found ${
        pet.name || pet._id
      } using "${foundWithMethod}"`
    );
    /* eslint-enable no-console */
  ...
  try {
    ...
    const searchMethods = [
      ...
      },
      {
        name: "Case insensitive _id",
        search: () =>
          Product.findOne({ _id: new RegExp(`^${productId}$`, "i") }),
      },
      {
        name: "Alternative fields",
        search: () =>
    ...
    for (let i = 0; i < searchMethods.length; i += 1) {
      try {
        ...
        if (product) {
          ...
          /* eslint-disable no-console */
          console.log(
            `✅ SUCCESS with "${foundWithMethod}": Found ${
    ...
    /* eslint-disable no-console */
    console.log(
      `✅ ENHANCED PRODUCT BYPASS SUCCESS: Found ${
        product.name || product._id
      } using "${foundWithMethod}"`
    );
    /* eslint-enable no-console */
  ...
  try {
    ...
    debugInfo.stats = {
      ...
      petsWithSimilarName: await Pet.countDocuments({
        name: new RegExp(petId.replace("p", "Pet ")),
      }),
    };
