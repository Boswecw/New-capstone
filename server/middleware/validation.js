...
const validatePetCreation = [
  body("name")
    ...
    .matches(/^[a-zA-Z0-9\s'-]+$/)
    .withMessage(
      "Pet name can only contain letters, numbers, spaces, apostrophes, and hyphens"
    ),
  ...
  body("type")
    ...
    .withMessage(
      "Pet type must be one of: dog, cat, bird, fish, rabbit, hamster, small-pet, reptile, other"
    ),
  ...
  body("adoptionFee")
    ...
    .isFloat({ min: 0, max: 99999 })
    .withMessage(
      "Adoption fee must be a positive number and cannot exceed 99,999"
    ),
...
const validatePetId = [
  param("id").custom((value) => {
    ...
    if (!isValidObjectId(value) && !isValidCustomPetId(value)) {
      throw new Error(
        "Invalid pet ID format. Must be a MongoDB ObjectId or custom pet ID (e.g., p001, p028)"
      );
    }
...
const validateProductId = [
  param("id").custom((value) => {
    ...
    if (!isValidObjectId(value) && !isValidCustomProductId(value)) {
      throw new Error(
        "Invalid product ID format. Must be a MongoDB ObjectId or custom product ID (e.g., prod_001)"
      );
    }
...
const validatePetQuery = [
  ...
  query("type")
    ...
    .withMessage(
      "Type must be dog, cat, bird, fish, rabbit, hamster, small-pet, reptile, or other"
    ),
  ...
  query("sort")
    ...
    .withMessage(
      "Sort must be name, price-low, price-high, type, newest, oldest, popular, or createdAt"
    ),
...
const validateProductQuery = [
  query("category")
    ...
    .withMessage(
      "Category must be general, dog, cat, bird, fish, small-pet, or other"
    ),
  ...
  query("sort")
    ...
    .withMessage(
      "Sort must be name, price-low, price-high, brand, newest, oldest, or popular"
    ),
