...
router.get('/dashboard', handleAsyncError(async (req, res) => {
  ...
  const recentPets = await Pet.find().sort({ createdAt: -1 }).limit(5).select('name type status createdAt');
  const recentContacts = await Contact.find().sort({ createdAt: -1 }).limit(5).select('name email subject status createdAt');
  ...
  const dashboardData = {
    stats: {
      ...
      products: productStats[0] || { totalProducts: 0, inStockProducts: 0, outOfStockProducts: 0 }
    },
    recentActivities: {
      pets: recentPets,
...
router.get('/users', handleAsyncError(async (req, res) => {
  const {
    search,
    role,
    isActive,
...
router.get('/pets', handleAsyncError(async (req, res) => {
...
router.post('/pets', validatePetCreation, handleAsyncError(async (req, res) => {
...
router.put('/pets/:id', validateObjectId, validatePetUpdate, handleAsyncError(async (req, res) => {
...
router.delete('/pets/:id', validateObjectId, handleAsyncError(async (req, res) => {
...
router.get('/products', handleAsyncError(async (req, res) => {
  const {
    search,
    category,
    brand,
  ...
  console.log(`✅ Found ${totalProducts} products, returning page ${pageNum}/${totalPages}`);
...
router.post('/products', handleAsyncError(async (req, res) => {
...
router.put('/products/:id', validateObjectId, handleAsyncError(async (req, res) => {
...
router.delete('/products/:id', validateObjectId, handleAsyncError(async (req, res) => {
...
router.get('/products/stats', handleAsyncError(async (req, res) => {
...
router.get('/contacts', handleAsyncError(async (req, res) => {
  const {
    status,
    search,
    limit = 20,
...
router.put('/contacts/:id', validateObjectId, handleAsyncError(async (req, res) => {
...
router.delete('/contacts/:id', validateObjectId, handleAsyncError(async (req, res) => {
...
router.get('/analytics', handleAsyncError(async (req, res) => {
  ...
  const topPages = [
    ...
    { page: '/products', visits: Math.floor(Math.random() * 600) + 300, bounceRate: '30%' },
    { page: '/about', visits: Math.floor(Math.random() * 400) + 200, bounceRate: '35%' },
  ...
  const userFlow = [
    ...
    { step: 'Browse Pets', users: Math.floor(totalUsers * 0.75), percentage: '75%' },
    { step: 'View Pet Details', users: Math.floor(totalUsers * 0.45), percentage: '45%' },
    { step: 'Contact/Apply', users: totalContacts, percentage: `${totalUsers > 0 ? Math.floor((totalContacts / totalUsers) * 100) : 0}%` },
    { step: 'Successful Adoption', users: petsAdopted, percentage: `${totalUsers > 0 ? Math.floor((petsAdopted / totalUsers) * 100) : 0}%` }
  ];
  ...
  const analyticsData = {
    overview: {
      ...
      conversionRate: totalContacts > 0 ? ((petsAdopted / totalContacts) * 100).toFixed(1) : '0.0',
      recentActivity: {
        pets: recentPets,
...
router.get('/reports', handleAsyncError(async (req, res) => {
      ...
      reportData = await Pet.aggregate([
        ...
        },
        {
          $group: {
            _id: '$status',
            count: { $sum: 1 },
            pets: { $push: { name: '$name', type: '$type', breed: '$breed' } }
          }
        }
      ]);
      break;
